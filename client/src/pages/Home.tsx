...
export default function Home() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#090b10] text-[#f4f5f7] selection:bg-violet-500/30">
              ...
              return (
                <article key={model.id} className="model-card group relative overflow-hidden rounded-2xl border border-white/[0.075] bg-[#10131b]/90 p-5 transition duration-300 hover:-translate-y-1 hover:border-violet-300/25 hover:shadow-[0_18px_60px_rgba(0,0,0,.25)]" style={{ animationDelay: `${index * 60}ms` }}>
                  ...
                  <div className="relative mt-5 flex items-center gap-3 border-t border-white/[0.06] pt-4">
                    <button onClick={() => startDownload(model)} disabled={Boolean(downloadingId && !isDownloading)} className={`relative flex h-9 flex-1 items-center justify-center gap-2 overflow-hidden rounded-lg text-[11px] font-black transition active:scale-[.98] ${isDownloaded ? "bg-emerald-400/10 text-emerald-300 hover:bg-emerald-400/15" : ollamaConnected ? "bg-violet-500 text-white shadow-[0_8px_20px_rgba(124,92,255,.18)] hover:bg-violet-400 disabled:cursor-not-allowed disabled:opacity-40" : "bg-amber-400/10 text-amber-200 hover:bg-amber-400/15 disabled:cursor-not-allowed disabled:opacity-40"}`}>
                      {isDownloading && <span className="absolute inset-y-0 right-0 bg-white/10 transition-all" style={{ width: `${downloadProgress}%` }} />}
                      <span className="relative flex items-center gap-2">{isDownloading ? <><span className="h-3 w-3 animate-spin rounded-full border-2 border-white/30 border-t-white" />{downloadProgress}% · {downloadStatus || "جارٍ التنزيل"}</> : isRunning ? <><Play className="h-3.5 w-3.5" />قيد التشغيل الآن</> : isDownloaded ? <><Check className="h-3.5 w-3.5" />محمّل على جهازك</> : ollamaConnected ? <><ArrowDownToLine className="h-3.5 w-3.5" />تنزيل النموذج</> : <><Settings2 className="h-3.5 w-3.5" />إعداد Ollama أولًا</>}</span>
                    </button>
      ...
      {showOllamaSetup && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setShowOllamaSetup(false)}><div className="w-full max-w-lg rounded-2xl border border-amber-300/15 bg-[#11141c] p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><div className="mb-2 flex items-center gap-2 text-amber-300"><Settings2 className="h-4 w-4" /><span className="text-xs font-black">إعداد Ollama مطلوب</span></div><h3 className="font-display text-xl font-bold text-white">التنزيل يتم على جهازك</h3></div><button onClick={() => setShowOllamaSetup(false)} className="rounded-lg p-2 text-slate-500 hover:bg-white/[0.06] hover:text-white"><X className="h-4 w-4" /></button></div><p className="mt-4 text-sm leading-6 text-slate-400">المنصة لا تنزّل النماذج إلى خادم الموقع. التنزيل يتم داخل Ollama المثبّت على جهازك، لذلك يجب تشغيله والسماح للمنصة بالاتصال.</p><div className="mt-5 space-y-3"><div><div className="mb-1.5 text-[10px] font-bold text-slate-600">1. ثبّت Ollama</div><div className="rounded-lg bg-[#0b0e14] px-3 py-2 text-xs text-cyan-300">https://ollama.com/download</div></div><div><div className="mb-1.5 text-[10px] font-bold text-slate-600">2. شغّله مع السماح لهذا الموقع</div><div className="rounded-lg bg-[#0b0e14] px-3 py-2 font-mono text-[11px] text-cyan-300">OLLAMA_ORIGINS="https://model-lab-arabic.onrender.com" ollama serve</div></div><div><div className="mb-1.5 text-[10px] font-bold text-slate-600">3. اضغط إعادة الفحص</div><div className="rounded-lg border border-white/[0.06] bg-white/[0.025] px-3 py-2 text-[11px] leading-5 text-slate-500">بعد تشغيل الأمر، أغلق هذه النافذة واضغط الزر مرة أخرى. سيظهر التقدم الحقيقي أثناء تنزيل النموذج.</div></div></div><div className="mt-5 flex gap-2"><button onClick={() => { setShowOllamaSetup(false); window.location.reload(); }} className="flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-amber-400 text-xs font-black text-[#241706] hover:bg-amber-300"><CheckCircle2 className="h-4 w-4" />إعادة الفحص</button><button onClick={() => setShowOllamaSetup(false)} className="h-10 rounded-xl border border-white/[0.08] px-4 text-xs font-bold text-slate-400 hover:bg-white/[0.05]">إغلاق</button></div></div></div>}
    </div>
  );
}
