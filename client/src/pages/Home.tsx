...
export default function Home() {
  ...
  const [downloaded, setDownloaded] = useState<string[]>(["llama-3-2"]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setDownloadProgress((current) => {
        const next = Math.min(current + 16, 100);
        if (next === 100) {
          setDownloaded((items) => (items.includes(downloadingId) ? items : [...items, downloadingId]));
          setDownloadingId(null);
          toast.success("اكتمل تنزيل النموذج", { description: "أصبح النموذج جاهزًا للتشغيل المحلي." });
        }
        return next;
      });
    }, 520);
  ...
  const startDownload = (model: Model) => {
    ...
    toast.message(`جاري تنزيل ${model.name}`, { description: `الحجم التقريبي ${model.size}` });
  };
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1600px] flex-row-reverse">
        <aside className="hidden w-[252px] shrink-0 border-l border-white/[0.07] bg-[#0c0f15]/80 px-5 py-6 md:flex md:flex-col">
          ...
          <div className="mt-auto rounded-2xl border border-white/[0.07] bg-gradient-to-br from-[#171327] to-[#11141b] p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] font-bold text-emerald-300">متصل</span>
            </div>
            <div className="text-xs font-bold text-slate-200">التشغيل المحلي مفعّل</div>
            <div className="mt-1 text-[11px] leading-5 text-slate-500">لا توجد مفاتيح API أو تكاليف استخدام.</div>
        ...
        <main className="min-w-0 flex-1 px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
          ...
          <div className="mb-7 flex items-end justify-between gap-4">
            <div>
              <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_8px_#6ee7b7]" />كل شيء يعمل محليًا</div>
          ...
          <section className="mb-8 grid gap-3 sm:grid-cols-3">
            {[
              { label: "النماذج المحمّلة", value: "01", detail: "من أصل 24 متاح", icon: Download, tone: "violet" },
              { label: "مساحة التخزين", value: "2.0 GB", detail: "من 48 GB متاحة", icon: HardDrive, tone: "cyan" },
              { label: "حالة المعالج", value: "جاهز", detail: "استخدام منخفض", icon: Cpu, tone: "emerald" },
              ...
              return (
                <article key={model.id} className="model-card group relative overflow-hidden rounded-2xl border border-white/[0.075] bg-[#10131b]/90 p-5 transition duration-300 hover:-translate-y-1 hover:border-violet-300/25 hover:shadow-[0_18px_60px_rgba(0,0,0,.25)]" style={{ animationDelay: `${index * 60}ms` }}>
                  ...
                  <div className="relative mt-5 flex items-center gap-3 border-t border-white/[0.06] pt-4">
                    <button onClick={() => startDownload(model)} disabled={Boolean(downloadingId && !isDownloading)} className={`flex h-9 flex-1 items-center justify-center gap-2 rounded-lg text-[11px] font-black transition active:scale-[.98] ${isDownloaded ? "bg-emerald-400/10 text-emerald-300 hover:bg-emerald-400/15" : "bg-violet-500 text-white shadow-[0_8px_20px_rgba(124,92,255,.18)] hover:bg-violet-400 disabled:cursor-not-allowed disabled:opacity-40"}`}>
                      {isDownloading ? <><span className="h-3 w-3 animate-spin rounded-full border-2 border-white/30 border-t-white" />{downloadProgress}%</> : isDownloaded ? <><Check className="h-3.5 w-3.5" />محمّل على جهازك</> : <><ArrowDownToLine className="h-3.5 w-3.5" />تنزيل النموذج</>}
                    </button>
          ...
          <section className="mt-8 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-6"><div className="mb-4 flex items-center justify-between"><h3 className="text-sm font-bold text-white">نشاط الجهاز</h3><span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" />مباشر</span></div><div className="space-y-4"><div><div className="mb-2 flex justify-between text-[11px]"><span className="text-slate-500">المعالج</span><span className="font-bold text-slate-300">18%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-[18%] rounded-full bg-gradient-to-l from-violet-400 to-indigo-500" /></div></div><div><div className="mb-2 flex justify-between text-[11px]"><span className="text-slate-500">الذاكرة</span><span className="font-bold text-slate-300">42%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-[42%] rounded-full bg-gradient-to-l from-cyan-300 to-blue-500" /></div></div><div><div className="mb-2 flex justify-between text-[11px]"><span className="text-slate-500">التخزين</span><span className="font-bold text-slate-300">4%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full w-[4%] rounded-full bg-gradient-to-l from-emerald-300 to-teal-500" /></div></div></div></div>
          </section>
